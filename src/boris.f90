...
contains
    ...
    !
    subroutine boris_step(r, v, q, m, dt, bfield) bind(c, name="boris_step_fortran")
        use iso_c_binding
        implicit none
        ! dimension
        ...
        real(c_double), dimension(N,N,N,d), intent(in) :: bfield
        ! helper variables
...
program test
    ...
    real(8) :: dt = 0.1d0
    real(8),dimension(3) :: r = [1.0d0, 0.0d0, 0.0d0], &
                            v = [0.0d0, 1.0d0, 0.0d0]
    real(8),dimension(16,16,16,3) :: b
    do i=1,16
        do j=1,16
            do k=1,16
                idx = transfer([i, j, k], 0.0d0)
